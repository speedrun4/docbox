...
if($request == NULL || $request->getType() != RequestType::DOCUMENT || $request->getClient() != $user->getClient())  {
	header("Location: login.php");
}
?>
<!DOCTYPE html>
...
<body class="fix-header card-no-border">
                ...
                <div class="row page-titles">
                    ...
                    <?php if($user->getProfile() == User::USER_ADMIN) { ?>
	                    ...
	                    <?php } else if($request->getStatus() != RequestStatus::COMPLETED && $request->getStatus() != RequestStatus::CANCELED) { ?>
		                    <div class="col-md-6 col-4 align-self-center">
		                        <a href="imprimir_pedido_documento.php?r=<?php echo $request->getId(); ?>" target='_blank' class="btn float-right hidden-sm-down btn-secondary">
                                	...
                                	<div class='row'>
                                		<div class='col-md-2'>
		                                    <div class="form-group">
		                                    	<div class='controls'>
			                                        ...
			                                        <input id='doc_box' name='doc_box' type="number" class="form-control inputDisabled" placeholder="0" min='1' value="<?php echo $request->getNumber(); ?>" disabled>
		                                    	</div>
		                                    </div>
                                		</div>
                                		...
                                		<div class='col-md-2'>
		                                    <div class="form-group">
		                                        ...
		                                        <input id='req_date' name='req_date' type="date" class="form-control inputDisabled" value="<?php echo $request->getDatetime()->format("Y-m-d"); ?>" disabled>
		                                    </div>
                                		</div>
                                		<div class='col-md-2'>
		                                    <div class="form-group">
		                                    	<div class='controls'>
			                                        ...
			                                        <input id='req_time' name='req_time' type="text" class="form-control inputDisabled" value="<?php echo $request->getDatetime()->format("H:i"); ?>" disabled>
		                                    	</div>
		                                    </div>
                                		</div>
                                		<div class='col-md-3'>
		                                    <div class="form-group">
		                                        ...
		                                        <input type="text" class="form-control inputDisabled" value="<?php echo $request->getUser()->getName(); ?>" disabled>
		                                    </div>
                                		</div>
                                		<div class='col-md-3'>
		                                    <div class="form-group">
		                                        <label for="req_status">Status</label>
		                                        <input id='req_status' type="text" class="form-control inputDisabled" value="<?php
					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								...
								<h6 class="card-subtitle">Documentos pertencentes ao pedido</h6>
								<div class="table-responsive m-t-40">
									<table id="data-table-basic" class="table table-bordered table-striped">
										<tbody>
											<?php
												$shouldShowCheckAll = FALSE;
											    $requestDocuments = $reqController->getRequestDocuments($request);
											    foreach($requestDocuments as $reqDoc) {
											    	$document = $reqDoc->getDocument();
											    	$isInStock = ($document->getRequest() == NULL || 
											    		(
											    			$document->getRequest() != NULL && RequestStatus::isInStock($reqDoc->getStatus())
											    		)) && $document->getBox()->getRequest() == NULL;

											    	$shouldShowCheckAll = !$isInStock || $shouldShowCheckAll;
											    	?>
    											    <tr>
        											    <td><?= $reqDoc-> getId() ?></td>
        												<td><?= $isInStock ? "" : $document->getBox()->getId() ?></td>
        												...
        												<td><?= $document->getFormat() == AbstractDocumentFormat::BOOK ? "" : $document->getNumber() ?></td>
        												<td><?= $document->getFormat() == AbstractDocumentFormat::BOOK ? $document->getNumFrom() : "" ?></td>
        												<td><?= $document->getFormat() == AbstractDocumentFormat::BOOK ? $document->getNumTo() : "" ?></td>
        												<td><?= $document->getYear() ?></td>
        												<td><?= $document->getFormat() == AbstractDocumentFormat::BOOK ? "" : $document->getLetter() ?></td>
        												<td><?= $document->getVolume() ?></td>
        												<td><?= $document->getFormat() == AbstractDocumentFormat::BOOK ? "" : $document->getCompany() ?></td>
        												<td><?php if($document->getFormat() != AbstractDocumentFormat::BOOK) {
        												    if($document->getDate() != NULL) {
        												        echo $document->getDate()->format('d/m/Y');
        												    }
        												}?></td>
        												<td><?= ($document->getRequest() || $document->getBox()->getRequest()) ? "Em pedido" : COMPANY_NAME ?></td>
        												<td>Arquivo</td>
    												</tr>
											    <?php }
											?>
										...
										<thead>
											<tr>
												<th>#</th>
												<th>
													<?php if($shouldShowCheckAll) { ?>
													<div class="custom-control custom-checkbox mr-sm-2">
						                            	<input type="checkbox" class="custom-control-input" id="checkAllDocs" name="checkAllDocs" value="on">
						                            	<label class="custom-control-label" for="checkAllDocs"></label>
						                            </div>
						                            <?php } else {?>
						                            #
						                            <?php } ?>
    ...
    $(function() {
		 ...
		 * @var boolean $returnDocState
		 */
		$returnDocState = $request->getType() == RequestType::DOCUMENT && 
			($request->getStatus() == RequestStatus::ATTENDEND || ($request->getStatus() == RequestStatus::RETURNING && $shouldShowCheckAll));
		?>
...
//         	"processing": true,
        	...
        	"aoColumnDefs" : [
            	{
                	...
                	"mRender": function(d,t,f) {
                        ...
                        return '<div class="custom-control custom-checkbox mr-sm-2">' +
                            '<input data-doc="' + f[0] + '" type="checkbox" class="check-request-add custom-control-input" id="check-doc-' + f[0] + 
                            '" name="check-doc-' + f[0] + '" value="on" >' +
                            '<label class="custom-control-label" for="check-doc-' + f[0] + '"></label>' +
                            '</div>';
                	}
                	<?php } else { ?>
					"visible" : false
	            	...
	            	$(".check-request-add").change(function() {
	                	...
	                	if($(this).is(":checked")) {
	                		if(listReturnDocIds.filter(function(value) {
	                			...
	                			listReturnDocIds.push($(this).data().doc);
	                		}
	                	} else {
	                		listReturnDocIds = listReturnDocIds.filter(function(value) { return parseInt(value) != parseInt(doc); });
	                	}
	                });
	            }
        	<?php } ?>
        ...
        $("#checkAllDocs").click(function() {
        	if($("#checkAllDocs").is(":checked")) {
	        	fetch('./core/actions/getDevolutionIds.php', {
	                method: 'POST',
	                headers : { 'Content-type': 'application/x-www-form-urlencoded' },
	                body: 'req=<?= $request->getId() ?>'
	            }).then((response) => response.json()).then(data => {
	            		data.ids.forEach(function(e,i,a) {
		        			if(listReturnDocIds.filter((value) => {return (parseInt(value) > 0 && parseInt(value) == parseInt(e))}).length == 0) {
			        			listReturnDocIds.push(parseInt(e));
			        		}
	                	});
	            	}).then(()=> {
	                	tbDocuments.draw();
        ...
        var tbReturns = $('#data-table-returns').DataTable({
        	...
        	"language" : _lang,
        	"lengthChange":false,
        	"sAjaxSource": "./core/data-tables/TableRequestReturns.php",
        	"sServerMethod": "POST",
        	"fnServerParams": function(aoData) {
				aoData.push({"name": "r", "value" : <?php echo $request->getId(); ?>});
        	},
            	...
            	swal({
		        	...
		        	if(result.value) {
		        		swal({
				        	...
				        	onOpen: function() {
				        		swal.showLoading();
	            	        	changeRequestStatus(<?php echo RequestStatus::RETURNED; ?>, null);
				        	}
			        	});
			        }
    	        });
	    			...
	    			swal({
			        	...
			        	if(result.value) {
	    	        		swal({
	    			        	...
	    			        	onOpen: function() {
	    			        		swal.showLoading();
	    			        		$.post("./core/actions/registerDocumentDevolution.php", {"ids" : listReturnDocIds},
	    	    			        	function(data) {
		    		  			        	if(data.ok) {
        	...
        	swal({
            	...
            	if(result.value) {
	        		swal({
			        	...
			        	onOpen: function() {
			        		swal.showLoading();
	        	        	changeRequestStatus(<?php echo RequestStatus::SENT; ?>, null);
			        	}
		        	});
            	}
	        });
        ...
        $("#btAttendedRequest").click(function() {
        	if($("#req_file").val() != "" && $("#req_file")[0].files.length == 1) {
				...
				if(fileOK) {
			      	runSave(<?php echo RequestStatus::ATTENDEND; ?>, null);
				}
        	} else {
        ...
        $("#btFinishRequest").click(function() {
        	if($("#req_file").val() != "" && $("#req_file")[0].files.length == 1) {
	        	if(belowMaxUploadSize('req_file')) {
	        		$("#req_file").parents(".form-group").removeClass("error");
	        		runSave(<?php echo RequestStatus::COMPLETED; ?>, null);
		        } else {
		        	$("#req_file").parents(".form-group").find(".help-block").html("<ul role=\"alert\"><li>O arquivo deve ser menor que " + MAX_UPLOAD_SIZE + "Mb</li><li>");
		        	$("#req_file").parents(".form-group").addClass("error").removeClass("validate");
			    }
	        	...
	        	if(res.ok) {
		        	if(res.action == 'print') {
		        		swal({
                        	...
                        	if(result.value) {
	                    		var win = window.open("imprimir_pedido_documento.php?r=<?php echo $request->getId(); ?>", '_blank');
