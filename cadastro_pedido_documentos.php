...
<body class="fix-header card-no-border">
                ...
                <div class="row page-titles">
                    <div class="col-md-6 col-8 align-self-center">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Pedir documento</li>
                        </ol>
                    </div>
                </div>
                <!-- ============================================================== -->
					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								...
								<h6 class="card-subtitle">Clique no botão verde para adicionar o documento ao pedido</h6>
								...
								<div class='row'>
									<div class='col-md-12'>
										<button id='btConfirmRequest' class='btn btn-primary m-t-10 float-right'>Realizar pedido</button>
									</div>
								</div>
							</div>
						</div>
	...
	var listRequestDocIds = [];
    $(function() {
    	/// blockFormEnter('#form-register,#form-register-box');

    	var _language = {
        	    "sEmptyTable": "Nenhum documento adicionado",
        	    ...
        	    "sZeroRecords": "Nenhum documento adicionado",
        	    "sSearch": "Pesquisar",
        	    "oPaginate": {
        	        "sNext": "Próximo",
        	        "sPrevious": "Anterior",
        	...
        	var tbDocuments = $('#data-table-basic').DataTable({
                ...
                "fnServerParams" : function(aoData) {
    				aoData.push (
    					...
    					{"name": "doc_marks", 'value': listRequestDocIds},
            	...
            	"drawCallback" : function(settings) {
                	$(".check-request-add").change(function() {
                    	...
                    	if($(this).is(":checked")) {
                    		if(listRequestDocIds.filter(function(value) { return (parseInt(value) == parseInt(doc)) }).length == 0) {
                        		listRequestDocIds.push($(this).data().doc);
                    		}
                    	} else {
                     		listRequestDocIds = listRequestDocIds.filter(function(value) { return parseInt(value) != parseInt(doc); });
                    	}
                    });
                },
            	"aoColumnDefs" : [
            		{
            			...
            			'mRender':function(d,t,f) {
            				// Se já está em pedido
            				if(f[13] > 0 || f[14] > 0) {
                                return "";
                            }
                            let checked = '';
            				if(listRequestDocIds.filter(function(value) { return (parseInt(value) == parseInt(f[0])) }).length > 0) {
            					checked = 'checked';
            				}
                        ...
                        "orderable" : false,
                        "mRender" : function(d,t,f) {
                            if (f[16] == "" || f[16] == null || f[16] == 'null') return "";
                        	return "<a target='blank' href='" + f[16] + "' class='btn btn-circle btn-primary'><i class='fa fa-eye'></i></a>";
                        }
                	}
        		]
        });
        ...
        $("#btConfirmRequest").click(function() {
			if(listRequestDocIds.length > 0) {
				swal({
        			title: 'Deseja realmente realizar o pedido?',
        			text: "",
        			type: 'question',
        			...
        			if(result.value) {
	    				$.post("./core/actions/registerDocumentRequest.php", {"docs" : listRequestDocIds}, function(data) {
	  			        	if(data.ok) {
	  			        		window.location.href = "visualizar_pedido_documentos.php?r=" + data.req_id;
	  			        	} else {
