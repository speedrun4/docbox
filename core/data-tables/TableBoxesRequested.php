...
$sWhere = " WHERE box_dead=FALSE
		AND box_client = $client
		AND (req_status = " . RequestStatus::ATTENDEND . "
		OR req_status = " . RequestStatus::RETURNING. ")
		AND req_type = " . RequestType::BOX . " ";
...
$sQuery = "SELECT SQL_CALC_FOUND_ROWS " . str_replace(" , ", " ", implode(", ", $aColumns)) . " FROM $sTable
		LEFT JOIN caixas_devolucoes ON bre_box_requested = dop_id
		LEFT JOIN devolucoes ON bre_return = ret_id
...
$sQuery = "SELECT COUNT(" . $sIndexColumn . ") FROM $sTable
	INNER JOIN caixas ON box_id = dop_box
  WHERE box_dead=FALSE AND box_client = $client";
...
while ($aRow = mysqli_fetch_array($rResult)) {
    $row = array();
    for ($i = 0; $i < count($aColumns); $i ++) {
        if (strcasecmp("doc_date", $aColumns[$i]) == 0) {
            $date = "";
